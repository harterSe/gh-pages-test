...
function buildAnimeMarkup(anime) {
  return `<div class="anime_item"><img class='anime_image' src=${anime.image_url} />
        ...
        <p class='anime_description'>Score: ${anime.score} ${anime.premiered}</p></div>`
}
...
async function onIncrementButtonClickAsync() {
  document.querySelector("#anime_id_input").value++;

  await onOkButtonClickAsync();
}

async function onDecrementButtonClickAsync() {
  document.querySelector("#anime_id_input").value--;
